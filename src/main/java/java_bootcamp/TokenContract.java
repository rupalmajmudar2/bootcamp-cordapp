...
public class TokenContract implements Contract {
    ...
    public void verify(@NotNull LedgerTransaction tx) throws IllegalArgumentException {
        ...
        if (!(cmd.getValue() instanceof TokenContract.Commands.Issue)) throw new IllegalArgumentException("Cmd has to be of type Issue. Found: " + cmd.getValue());

        if (tx.getInputStates().size() != 0) throw new IllegalArgumentException(("Txn inputs should be 0. Found: " + tx.getInputStates().size()));
        if (tx.getOutputStates().size() != 1) throw new IllegalArgumentException(("Txn outputs should be 1. Found: " + tx.getOutputStates().size()));

        //(B) Content rules -
        ContractState cs= tx.getOutput(0);
        if (!(cs instanceof TokenState)) throw new IllegalArgumentException("Tx output must be a TokenState. Found: " + cs.getClass());
