...
app.controller('DemoAppController', function($http, $location, $uibModal) {
    ...
    demoApp.openModalForTokenTransfer = () => {
        console.log('In demoApp.openModalForTokenTransfer');
        const modalInstance = $uibModal.open({
    ...
    demoApp.openModalForAttachmentSend = () => {
            console.log('In demoApp.openModalForAttachmentSend');
        /*const modalInstance = $uibModal.open({
            templateUrl: 'demoAppModalAttachment.html',
            controller: 'ModalInstanceCtrl',openModalForAttachmentSend
            controllerAs: 'modalInstance',
            resolve: {
                demoApp: () => demoApp,
                apiBaseURL: () => apiBaseURL,
...
app.controller('ModalInstanceCtrl', function ($http, $location, $uibModalInstance, $uibModal, demoApp, apiBaseURL, peers) {
    ...
    modalInstance.sendAttachment = () => {
        console.log("In #sendAttachment 00");
        if (invalidFormInput()) {
            modalInstance.formError = true;
        } else {
            modalInstance.formError = false;
            ...
            const sendAttachmentEndpoint = `${apiBaseURL}send-attachment?filename=${modalInstance.form.fn}&newowner=${modalInstance.form.counterparty}`;

            // Create PO and handle success / fail responses.
            $http.put(sendAttachmentEndpoint).then(
                (result) => {
                    modalInstance.displayMessage(result);
                    demoApp.getTokens();
                },
